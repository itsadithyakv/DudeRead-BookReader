...
pub async fn read_book_bytes(
  ...
  if (ext == "mobi" || ext == "azw3") && !storage::converter_installed(&app) {
    return Err(
      "This book needs the optional AZW3/MOBI converter. Approve its download before opening the book."
        .to_string()
    );
  }
  ...
  tauri::async_runtime::spawn_blocking(move || {
    let readable_path = if ext == "epub" {
      ...
      storage::ensure_epub_version(&source_path, &file_hash, Some(&app_handle))
        .map_err(|e| e.to_string())?
        .ok_or_else(|| "The book conversion failed. Please retry from Settings.".to_string())?
    };
