...
fn oauth_client() -> Result<BasicClient> {
  let client_id = env::var("LEAFLET_GOOGLE_CLIENT_ID")
    .map_err(|_| anyhow!("Missing LEAFLET_GOOGLE_CLIENT_ID"))?;
  let client_secret = env::var("LEAFLET_GOOGLE_CLIENT_SECRET")
    .map_err(|_| anyhow!("Missing LEAFLET_GOOGLE_CLIENT_SECRET"))?;
